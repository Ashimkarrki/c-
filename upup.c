...
show(len);
   ...
   i=determine_index(id,len);
    ...
    gets(identy.name);
    feri:
    printf("Citizen id (like 39-01-76-01033):");
    gets(identy.citizen_id);
    if(strlen(identy.citizen_id)!=14)
    {
    printf("ID criteria not met\n");
    goto feri;
  ...
  system("clear");
   ...
   //     puts(datas[i].name);
    ...
    show_without_welcome(len);
        ...
        scanf("%d",&delete_id);
             printf("Is it %s?\n",datas[determine_index(delete_id,len)].name);
             scanf(" %c",&qq);
      }while(qq=='n');
      }
    for(int i=0;i<len;i++)
...
do{y++;
    ...
    datas[len+extra].id=get_highest(len)+1+y;
...
int total=0;
    ...
    printf("\nTotal %d ",total);
printf("\n*Regular customer are given 5-percent discount\n");
